...
from actions.SATAN_connect    import (
...
try:
    from SATAN_connect.service import get_service as get_SATAN_connect_service
except Exception:
    get_SATAN_connect_service = None
...
STARTUP_LOG     = Path(os.environ.get("LOCALAPPDATA", str(BASE_DIR))) / "SATAN" / "startup.log"
...
def _ensure_desktop_shortcut() -> None:
    ...
    try:
        ...
        shortcut_path = desktop_dir / "SATAN.lnk"
        ...
        ps1_script = "\n".join([
            ...
            "$Shortcut.Description = 'Launch SATAN'",
...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are SATAN, a calm, direct, and professional AI assistant. "
...
def _gemini_text_reply(prompt: str) -> str:
    ...
    system_prompt = (
        "You are SATAN, a concise, helpful desktop assistant. "
...
def _wakeword_detected(text: str) -> bool:
    ...
    if compact in phrases or any(p in compact for p in phrases):
        return True
    return any(word in {"satan", "hey", "hi", "hello"} for word in words)
...
def _update_memory_async(user_text: str, SATAN_text: str) -> None:
    global _last_memory_input

    user_text   = (user_text   or "").strip()
    SATAN_text = (SATAN_text or "").strip()
    ...
    try:
        api_key = _get_api_key()
        if not should_extract_memory(user_text, SATAN_text, api_key):
            return
        data = extract_memory(user_text, SATAN_text, api_key)
        if data:
            update_memory(data)
...
TOOL_DECLARATIONS = [
    ...
    },
    {
        ...
        "description": (
            "Lists devices connected to SATAN Connect. Use when the user asks what devices are connected, "
    ...
    },
    {
        ...
        "description": (
            "Routes a SATAN Connect command to a paired device through the gateway. "
    ...
    },
    {
        ...
        "description": (
            "Creates or approves SATAN Connect pairing. Use to generate a QR code / pairing code for a new device, "
    ...
    },
    {
        ...
        "description": (
            "Disconnects a device from SATAN Connect and marks it offline. "
    ...
    },
    {
        ...
        "description": (
            ...
            "SATAN automatically infers the best visual style from the topic, searches for a matching online template when available, "
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "theme": {
                    "type": "STRING",
                    "description": "Optional presentation theme or visual direction such as neon, corporate, luxury, academic, sunset, or creative. If omitted, SATAN infers the best style automatically."
                },
                "outline": {
                    "type": "STRING",
    ...
    },
    {
        ...
        "close the assistant, say goodbye, or stop SATAN. "
...
class SatanLive:
    ...
    def __init__(self, ui: SatanUI, dashboard=None, dashboard_started: bool = False, enable_dashboard: bool = True):
        self.ui             = ui
    ...
    def _on_text_command(self, text: str, source: str = "local"):
        ...
        try:
            ...
            if routed_text_home != text:
                print(f"[SATAN] Redirection: '{text}' -> '{routed_text_home}'")
                text = routed_text_home
        except Exception as e:
            print(f"[SATAN] Redirection error: {e}")
        ...
        try:
            ...
            pass
        if self._handle_SATAN_connect_command(text, source=source or "local"):
            return
        ...
        if _looks_like_screen_request(text):
            try:
                self.ui.update_task_workspace(
                    ...
                    output="SATAN is inspecting the screen for what you asked about.",
                    percent=40,
                )
            except Exception:
                pass
    ...
    def _handle_smart_home_command(self, text: str, source: str = "local") -> bool:
        ...
        connect_words = (
            "phone", "mobile", "android", "tablet", "device", "devices", "SATAN connect",
        ...
        try:
            ...
            self.ui.write_log(f"SATAN: {detail}")
            self.speak(detail)
    ...
    def _handle_SATAN_connect_command(self, text: str, source: str = "local") -> bool:
        normalized = re.sub(r"\s+", " ", re.sub(r"[^a-z0-9\s%]", " ", text.lower())).strip()
        connect_words = (
            "phone", "mobile", "android", "tablet", "device", "devices", "SATAN connect",
        ...
        try:
            ...
            if result.get("success", False):
                ...
                title = f"SATAN Connect: {action}"
                self.ui.update_task_workspace(
                    title=title,
                    command=text,
                    plan=[
                        "Identify the paired phone or device",
                        "Route the command through SATAN Connect",
            ...
            self.ui.write_log(f"ERR: SATAN Connect command failed: {result.get('error') or 'Unknown error'}")
            return False
        except Exception:
            return False
    ...
    def _on_meeting_update(self, payload: dict):
        ...
        if answer:
            self.ui.write_log(f"SATAN: {answer}")
    ...
    def _handle_attention_response(self, text: str) -> bool:
        ...
        if kind == "message":
            ...
            if self._attention_matches(lower, ("hear", "read", "what is it", "tell me", "show it", "open it")):
                ...
                self.ui.write_log(f"SATAN: {preview}")
    ...
    def _fallback_reply(self, text: str, memory_ctx: str = ""):
        try:
            ...
            try:
                self.ui.update_task_workspace(
                    status="Thinking",
                    output="SATAN is drafting a direct reply.",
                    percent=35,
                )
            except Exception:
                pass
            ...
            if gemini_first:
                try:
                    ...
                    print(f"[SATAN] ⚠️ Gemini fallback failed: {e}")
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        parts.append(
            "Wake-word mode: if the microphone is muted, still listen for the words 'SATAN', 'hey', 'hi', and 'hello'. "
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        if tool_voice:
            try:
                self.ui.write_log(f"SATAN: {tool_voice}")
            except Exception:
                pass
            try:
        ...
        print(f"[SATAN] 📤 {name} → {str(result)[:80]}")
    ...
    async def _listen_audio(self):
        print("[SATAN] 🎤 Mic started")
        loop = asyncio.get_event_loop()

        def callback(indata, frames, time_info, status):
            with self._speaking_lock:
                SATAN_speaking = self._is_speaking
            if self._phone_active:
                return
            if not SATAN_speaking and (not self.ui.muted or getattr(self.ui, "_wakeword_listening", False)):
                data = indata.tobytes()
                def _try_enqueue():
                    try:
                        ...
                        pass
                loop.call_soon_threadsafe(_try_enqueue)
    ...
    async def _receive_audio(self):
        ...
        try:
                ...
                async for response in self.session.receive():
                    ...
                    if response.server_content:
                        ...
                        if sc.turn_complete:
                            ...
                            if full_out:
                                self.ui.write_log(f"SATAN: {full_out}")
                            out_buf = []
            ...
            print(f"[SATAN] ❌ Recv: {e}")
            traceback.print_exc()
            raise

    async def _play_audio(self):
        print("[SATAN] 🔊 Play started")
        loop = asyncio.get_event_loop()
    ...
    async def run(self):
        ...
        while True:
                ...
                try:
                    async with asyncio.TaskGroup() as tg:
                        ...
                        try:
                            ...
                            pass
                        self.ui.set_state("LISTENING")
                        self.ui.write_log("SYS: SATAN online.")
            ...
            self.set_speaking(False)
            self.ui.set_state("LISTENING")
            print("[SATAN] 🔄 Reconnecting in 5s...")
            await asyncio.sleep(5)

def main():
    _startup_log("main entered")
    _ensure_desktop_shortcut()
    ui = SatanUI(str(BASE_DIR / "assets" / "SATAN_Lite_Logo.png"), show_immediately=True)
    dashboard = None
    ...
    if DashboardServer is not None and not dashboard_enabled:
        ...
        try:
            ui.write_log("SYS: Mobile Connect is already running in another SATAN instance.")
        except Exception:
            pass
    ...
    SATAN_connect = None
    SATAN_connect_enabled = False
    if get_SATAN_connect_service is not None:
        try:
            SATAN_connect = get_SATAN_connect_service(BASE_DIR)
            SATAN_connect_enabled = bool(SATAN_connect.gateway.config.enabled)
        except Exception as exc:
            _startup_log(f"SATAN connect init failed: {exc}")
            try:
                ui.write_log(f"ERR: SATAN Connect failed to initialize: {exc}")
            except Exception:
                pass
            SATAN_connect = None
    try:
        if SATAN_connect is not None and hasattr(ui, "set_SATAN_connect_service"):
            ui.set_SATAN_connect_service(SATAN_connect)
    except Exception:
        pass
    if SATAN_connect is not None and SATAN_connect_enabled:
        connect_port = int(getattr(SATAN_connect.gateway.config, "port", 8765))
        if _is_port_in_use(connect_port):
            _startup_log(f"SATAN connect disabled: port {connect_port} already in use")
            try:
                ui.write_log(f"SYS: SATAN Connect is already running on port {connect_port}.")
            except Exception:
                pass
        else:
            def _start_SATAN_connect_server():
                try:
                    _startup_log("SATAN connect thread started")
                    SATAN_connect.start_background()
                    _startup_log("SATAN connect thread spawned")
                except Exception as exc:
                    _startup_log(f"SATAN connect thread error: {exc}")
                    try:
                        ui.write_log(f"ERR: SATAN Connect server failed: {exc}")
                    except Exception:
                        pass

            threading.Thread(target=_start_SATAN_connect_server, daemon=True).start()
    ...
    def runner():
        ...
        try:
            if plugin_manager is not None:
                SATAN_echo.plugin_manager = plugin_manager
                plugin_manager.register_SATAN(SATAN_echo)
                # allow plugins to run a startup hook
                try:
                    plugin_manager.dispatch("on_startup", SATAN_echo)
                except Exception:
                    pass
        except Exception:
            pass
        try:
            asyncio.run(SATAN_echo.run())
