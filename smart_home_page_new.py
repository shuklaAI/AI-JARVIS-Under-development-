...
PANEL_2 = "#101116"
ACCENT = "#cc0000"
ACCENT_SOFT = "rgba(200, 0, 0,0.12)"
ACCENT_LINE = "rgba(200, 0, 0,0.22)"
TEXT = "#ffffff"
...
def _soft_button_style() -> str:
    return (
        ...
        f"QPushButton:hover {{ border: 1px solid {ACCENT}; background: rgba(200, 0, 0,0.08); }}"
    )
        ...
        if kind == "fan":
            ...
            for s in range(1, 6):
                ...
                if is_on and s <= current_speed:
                    seg.setStyleSheet("background: #cc0000; border-radius: 2.5px;")
                else:
...
class SatanHomePage(QWidget):
    def __init__(self, parent=None):
        ...
        self.setObjectName("SatanHomePageModern")
        self.setStyleSheet(f"QWidget#SatanHomePageModern {{ background: transparent; }} QScrollArea {{ background: transparent; border: none; }}")
    ...
    def _build_header(self, lay: QVBoxLayout):
        ...
        self._voice_button.setStyleSheet(f"QPushButton {{ background: rgba(255, 255, 255, 0.02); color: {TEXT}; border: 1px solid rgba(255,255,255,0.06); border-radius: 12px; font-size: 16px; }} QPushButton:hover {{ border: 1px solid {ACCENT}; background: rgba(200, 0, 0,0.08); }}")
    ...
    def _build_voice_card(self, lay: QVBoxLayout):
        ...
        self._mic_orb.setStyleSheet(f"QLabel {{ background: rgba(200, 0, 0,0.12); color: {TEXT}; border: 2px solid rgba(200, 0, 0,0.55); border-radius: 43px; }}")
        ...
        for _ in range(36):
            ...
            bar.setStyleSheet("background: rgba(200, 0, 0,0.20); border-radius: 2px;")
    ...
    def _build_my_devices(self, lay: QVBoxLayout):
        ...
        self._add_btn.setStyleSheet(f"QPushButton {{ background: transparent; color: {TEXT}; border: 1px solid {ACCENT}; border-radius: 12px; padding: 0 14px; }} QPushButton:hover {{ background: rgba(200, 0, 0,0.10); }}")
        ...
        self._empty_add_btn.setStyleSheet(f"QPushButton {{ background: rgba(200, 0, 0,0.12); color: {TEXT}; border: 1px solid {ACCENT}; border-radius: 12px; }} QPushButton:hover {{ background: rgba(200, 0, 0,0.18); }}")
    ...
    def _animate_voice(self):
        ...
        if hasattr(self, "_wave_bars") and self._wave_bars:
            for idx, bar in enumerate(self._wave_bars):
                ...
                bar.setStyleSheet(f"background: {'rgba(200, 0, 0,0.55)' if self._voice_state in ('Listening', 'Executing') else 'rgba(255,255,255,0.18)'}; border-radius: 2px;")
        if hasattr(self, "_voice_small_bars") and self._voice_small_bars:
            for idx, bar in enumerate(self._voice_small_bars):
                ...
                bar.setStyleSheet(f"background: {'rgba(200, 0, 0,0.35)' if self._voice_state in ('Listening', 'Executing') else 'rgba(255,255,255,0.15)'}; border-radius: 2px;")
        if hasattr(self, "_voice_cmd_lbl") and self._voice_cmd_lbl:
            self._voice_cmd_lbl.setText({"Idle": '"SATAN"', "Listening": '"Turn bedroom fan to speed 4"', "Thinking": '"Understanding..."', "Executing": '"Applying command..."', "Completed": '"Done"'}.get(self._voice_state, '"SATAN"'))
