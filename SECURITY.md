...
## SATAN Echo Security Overview

This repository includes a security overview for SATAN Echo's current runtime model, gateway exposure, and authentication flow.
...
- SATAN Echo uses Gemini as the primary AI provider and OpenRouter as a fallback.
...
### SATAN Connect gateway exposure

SATAN Connect is the local device gateway layer for SATAN Echo.

#### Configuration

- Default gateway config: `config/SATAN_connect.json`
...
- Optional mDNS discovery is provided through `SATAN_connect.gateway.discovery.GatewayDiscovery` and Zeroconf.
- Discovery advertises service `_SATAN._tcp.local.` only when the Zeroconf library is installed and `advertise` is enabled.
...
- Pairing uses a temporary pairing offer created by `SATAN_connect.gateway.pairing.PairingManager`.
...
- Device records are stored in `config/SATAN_connect/devices.json`.
...
- Use OS firewall rules to restrict access to port `8765` when SATAN Connect is enabled.
- Disable `advertise` in `config/SATAN_connect.json` unless discovery is needed.
- Revoke lost or untrusted devices using `/gateway/devices/{device_id}/revoke`.
- Run SATAN Echo on a trusted local network.
