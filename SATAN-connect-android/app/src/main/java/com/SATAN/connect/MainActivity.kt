...
import com.SATAN.connect.ui.SATANConnectApp
import com.SATAN.connect.ui.theme.SATANConnectTheme
...
class MainActivity : ComponentActivity() {
    ...
    private val notificationPermission = registerForActivityResult(ActivityResultContracts.RequestPermission()) { granted ->
        if (granted || Build.VERSION.SDK_INT < Build.VERSION_CODES.TIRAMISU) {
            ...
            pendingServiceStart = false
            AgentStateStore.setError("Notification permission is required for SATAN Connect.")
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        storage.loadGatewayHint()?.let {
            ...
            AgentStateStore.setGateway(
                com.SATAN.connect.core.GatewayEndpoint(
                    name = "SATAN PC",
                    host = it.host,
                    port = it.port,
                )
    ...
    private fun startGatewayService() {
        val endpoint = AgentStateStore.gateway.value
        if (endpoint != null || AgentStateStore.credential.value != null) {
            val intent = Intent(this, SATANConnectForegroundService::class.java)
