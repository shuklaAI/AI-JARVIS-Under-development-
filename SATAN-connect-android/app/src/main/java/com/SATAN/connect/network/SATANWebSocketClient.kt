...
class SATANWebSocketClient(
    ...
    private fun sendHello() {
        send(SATANProtocol.hello(buildSnapshot()))
    ...
    private fun sendPairRequest() {
        ...
        send(SATANProtocol.envelope(SATANProtocol.PAIR_REQUEST, snapshot))
    ...
    private fun sendAuthenticate() {
        ...
        currentCredential = credential
        send(SATANProtocol.authenticate(credential))
    ...
    private fun handleCommandMessage(root: JSONObject) {
        ...
        val response = SATANProtocol.envelope(
            SATANProtocol.RESULT,
            result.toJson(),
            requestId = requestId,
        )
        send(response)
    ...
    private inner class SATANSocketListener : WebSocketListener() {
        ...
        override fun onMessage(webSocket: WebSocket, text: String) {
            runCatching {
                ...
                when (type) {
                    SATANProtocol.PAIR_REQUEST -> {
                        AgentStateStore.setStatus("Waiting for approval")
                    }
                    SATANProtocol.PAIR_APPROVED -> handlePairApproved(root)
                    SATANProtocol.DEVICE_ONLINE -> {
                        AgentStateStore.setConnectionState(ConnectionState.CONNECTED)
                        AgentStateStore.setStatus("Connected")
                    }
                    SATANProtocol.CAPABILITIES -> {
                        AgentStateStore.addLog("Capabilities synced")
                    }
                    SATANProtocol.EXECUTE -> handleCommandMessage(root)
                    SATANProtocol.PING -> {
                        send(SATANProtocol.envelope(SATANProtocol.PONG, JSONObject().put("status", "ok"), requestId = root.optString("request_id")))
                    }
                    SATANProtocol.ERROR -> {
                        AgentStateStore.setError(root.optJSONObject("payload")?.optString("error"))
                    }
                    SATANProtocol.CHAT_MESSAGE -> {
    ...
    fun sendChatMessage(text: String) {
        val payload = SATANProtocol.chatMessage(text)
