...
import com.SATAN.connect.SATANConnectForegroundService
import com.SATAN.connect.core.AgentStateStore
import com.SATAN.connect.core.ConnectionState
import com.SATAN.connect.core.DeviceCredential
import com.SATAN.connect.core.GatewayEndpoint
import com.SATAN.connect.core.PairingOffer
import com.SATAN.connect.network.SATANGatewayDiscovery
import com.SATAN.connect.pairing.PairingPayloadParser
...
fun SATANConnectApp(
    ...
    val discovery = remember { SATANGatewayDiscovery(context) }
        ...
        NavHost(
                ...
                if (pairingOffer != null) {
                    ...
                    ManualConnectDialog(
                        ...
                        onConnect = {
                            ...
                            AgentStateStore.setGateway(GatewayEndpoint(name = "SATAN PC", host = manualHost.trim(), port = p))
            ...
            composable("scanner") {
                QrScannerScreen(
                    ...
                    onScanned = { raw ->
                        ...
                        if (offer == null) {
                            scanError = "That QR code does not look like a SATAN pairing code."
                            AgentStateStore.setError(scanError)
                        } else {
                            scanError = null
                            storage.saveGatewayHint(offer)
                            AgentStateStore.setPairingOffer(offer)
                            AgentStateStore.setGateway(GatewayEndpoint(name = "SATAN PC", host = offer.host, port = offer.port))
            ...
            composable("home") {
                if (credential != null) {
                    ConnectedScreen(
                        ...
                        status = status,
                        onDisconnect = {
                            context.startService(Intent(context, SATANConnectForegroundService::class.java).apply {
                                action = SATANConnectForegroundService.ACTION_STOP
                            })
            ...
            composable("chat") {
                ChatScreen(
                    onBack = { navController.popBackStack() },
                    onSendMessage = { text ->
                        val intent = Intent(context, SATANConnectForegroundService::class.java).apply {
                            action = SATANConnectForegroundService.ACTION_SEND_CHAT
                            putExtra(SATANConnectForegroundService.EXTRA_CHAT_TEXT, text)
                        }
...
fun WelcomeScreen(onNext: () -> Unit) {
    Column(modifier = Modifier.fillMaxSize(), verticalArrangement = Arrangement.Center, horizontalAlignment = Alignment.CenterHorizontally) {
        Text("SATAN", style = MaterialTheme.typography.displayLarge, fontWeight = FontWeight.Black, color = MaterialTheme.colorScheme.primary)
...
fun AboutScreen(onNext: () -> Unit) {
    Column(modifier = Modifier.fillMaxSize(), verticalArrangement = Arrangement.Center, horizontalAlignment = Alignment.CenterHorizontally) {
        GlassCard {
            ...
            Text("This agent acts as a secure physical bridge between your Android device and your PC's SATAN AI.", color = androidx.compose.ui.graphics.Color.White, textAlign = androidx.compose.ui.text.style.TextAlign.Center)
            Spacer(modifier = Modifier.height(24.dp))
            Button(onClick = onNext, modifier = Modifier.fillMaxWidth()) {
                Text("Next")
            }
...
private fun StartupPermissionsScreen(
    ...
    Column(
        ...
        Text("Enable the permissions SATAN Connect needs to control your phone reliably.", color = MaterialTheme.colorScheme.onSurfaceVariant)
...
private fun DiscoveryScreen(
    ...
    Column(
        ...
        Text("SATAN CONNECT", style = MaterialTheme.typography.headlineMedium, fontWeight = FontWeight.Black, color = androidx.compose.ui.graphics.Color.White)
        Spacer(Modifier.height(8.dp))
        Text("Connect this device to SATAN AI.", color = MaterialTheme.colorScheme.onSurfaceVariant)
        ...
        Button(onClick = onFindSATAN, modifier = Modifier.fillMaxWidth()) { Text("Find SATAN") }
        Spacer(Modifier.height(12.dp))
...
private fun PairingPendingScreen(
    ...
    Column(Modifier.fillMaxSize().padding(24.dp), verticalArrangement = Arrangement.Center) {
        Text("CONNECT TO SATAN?", style = MaterialTheme.typography.headlineSmall, fontWeight = FontWeight.Black)
        Spacer(Modifier.height(8.dp))
        Text("${gateway?.name ?: "SATAN PC"}\n${gateway?.host ?: offer.host}:${offer.port}", color = MaterialTheme.colorScheme.onSurfaceVariant)
        Spacer(Modifier.height(20.dp))
        Card(colors = CardDefaults.cardColors(containerColor = androidx.compose.ui.graphics.Color.Black.copy(alpha = 0.55f), contentColor = androidx.compose.ui.graphics.Color.White), border = androidx.compose.foundation.BorderStroke(1.dp, MaterialTheme.colorScheme.primary.copy(alpha = 0.3f))) {
...
private fun ConnectedScreen(
    ...
    Column(
        ...
        Text("SATAN CONNECT", style = MaterialTheme.typography.headlineSmall, fontWeight = FontWeight.Black, color = androidx.compose.ui.graphics.Color.White)
        Spacer(Modifier.height(8.dp))
        Text("● Connected", color = MaterialTheme.colorScheme.primary)
        Spacer(Modifier.height(20.dp))
        Card(colors = CardDefaults.cardColors(containerColor = androidx.compose.ui.graphics.Color.Black.copy(alpha = 0.55f), contentColor = androidx.compose.ui.graphics.Color.White), border = androidx.compose.foundation.BorderStroke(1.dp, MaterialTheme.colorScheme.primary.copy(alpha = 0.3f))) {
            Column(Modifier.padding(16.dp)) {
                Text(gateway?.name ?: "SATAN PC", fontWeight = FontWeight.Bold)
        ...
        Card(
            ...
            Row(
                ...
                Column {
                    Text("SATAN Chat", fontWeight = FontWeight.Bold, style = MaterialTheme.typography.titleMedium)
                    Spacer(Modifier.height(4.dp))
