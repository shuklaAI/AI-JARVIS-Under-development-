...
def _find_logo_path() -> Path | None:
    ...
    candidates = [
        assets_dir / "SATAN_Lite_Logo.png",
        assets_dir / "SATAN_Lite_Logo..png",
        assets_dir / "SATAN_Lite_Logo.ico",
    ]
    for candidate in candidates:
        if candidate.exists():
            return candidate
    for candidate in sorted(assets_dir.glob("SATAN_Lite_Logo*")):
        if candidate.is_file():
            return candidate
    return None

LOGO_FILE  = _find_logo_path() or BASE_DIR / "assets" / "SATAN_Lite_Logo.png"
...
class C:
    ...
    BORDER_B  = "rgba(200, 0, 0, 0.20)"
    BORDER_A  = "rgba(200, 0, 0, 0.15)"
    PRI       = "#cc0000"
    PRI_DIM   = "#8b0000"
    PRI_GHO   = "rgba(200, 0, 0, 0.15)"
    ACC       = "#cc0000"
    ACC2      = "#ff3333"
    GREEN     = "#ff1a1a"
    GREEN_D   = "#b30000"
...
class RemoteKeyOverlay(QWidget):
    ...
    def __init__(self, url: str, key: str, auto: str, manual: str, parent=None):
        ...
        subtitle = QLabel("Scan the QR code with your phone to remotely control SATAN.")
    ...
    def mark_connected(self) -> None:
        ...
        self._qr_label.setStyleSheet("color: #ff1a1a; background: #041006; border-radius: 12px;")
        self._timer_lbl.setText("Phone connected. SATAN remote is ready.")
...
def _logo_pixmap(size: int) -> QPixmap:
    ...
    if pix.isNull() and LOGO_ICO.exists():
        pix = QPixmap(str(LOGO_ICO))
    if pix.isNull():
    ...
    def __init__(self, parent=None):
        ...
        self.setStyleSheet(
            ...
            QFrame#GestureCameraPreview {{
                ...
                border: 1px solid rgba(200, 0, 0, 0.24);
                border-radius: 16px;
            }}
        ...
        self._sensitivity_select.setStyleSheet(
            "QComboBox { background: rgba(255,255,255,0.05); color: #f4f6f8; border: 1px solid rgba(200, 0, 0,0.24); border-radius: 8px; padding: 4px 8px; }"
...
class TaskCard(QFrame):
    def __init__(self, parent=None):
        ...
        self._plan_lbl = QLabel("Plan: SATAN will generate a task plan after you send a command.")
    ...
    def set_task(self, title: str, desc: str, percent: int):
        ...
        self._plan_lbl.setText("Plan: SATAN will generate a task plan after you send a command.")
    ...
    def __init__(self, title: str, subtitle: str = "", parent=None):
        ...
        self.setStyleSheet("""
            QFrame#AttachmentCard {
                ...
                border: 1px solid rgba(200, 0, 0,0.26);
                border-radius: 10px;
            }
        """)
        lay = QHBoxLayout(self)
        ...
        icon.setStyleSheet("color: #cc0000; background: rgba(255,255,255,0.03); border-radius: 13px; font-size: 14px;")
        lay.addWidget(icon)
        txt = QVBoxLayout()
...
class EventCard(QFrame):
    def __init__(self, title: str, detail: str, stamp: str, icon: str = "●", accent: str = "#cc0000", parent=None):
        super().__init__(parent)
...
class ArtifactCard(QFrame):
    def __init__(self, title: str, file_type: str = "File", status: str = "Generated", path: str = "", parent=None):
        ...
        badge.setStyleSheet("background: rgba(200, 0, 0,0.08); color: #cc0000; border: 1px solid rgba(200, 0, 0,0.24); border-radius: 15px;")
...
class ChatBubble(QFrame):
    def __init__(self, role: str, name: str, text: str, stamp: str, attachments: list[dict] | None = None, parent=None, animate: bool = False):
        ...
        if role == "assistant":
            avatar = _framed_logo(24, 24, bg="rgba(12,14,20,245)", border="rgba(200,0,0,0.50)", radius=12, inset=4)
            head.addWidget(avatar)
            name_lbl = QLabel(name or "SATAN")
            name_lbl.setFont(QFont("Segoe UI", 9, QFont.Weight.Bold))
            name_lbl.setStyleSheet("color: #ffffff; background: transparent;")
...
class HistoryConversationItem(QFrame):
    ...
    def __init__(self, conversation_id: str, title: str, stamp: str, pinned: bool = False, parent=None):
        ...
        icon.setAlignment(Qt.AlignmentFlag.AlignCenter)
        icon.setStyleSheet("background: rgba(200, 0, 0,0.10); color: #cc0000; border: 1px solid rgba(200, 0, 0,0.28); border-radius: 15px; font: 700 11pt 'Segoe UI';")
        lay.addWidget(icon)
        ...
        if pinned:
            pin = QLabel("PIN")
            pin.setStyleSheet("color: #ff1a1a; background: transparent; font: 700 7pt 'Courier New';")
            row.addWidget(pin)
        row.addStretch()
        stamp_lbl = QLabel(stamp)
        stamp_lbl.setFont(QFont("Segoe UI", 7))
...
class ConversationFeed(QScrollArea):
    ...
    def _build_event_card(self, text: str, stamp: str, event_type: str | None = None) -> QWidget:
        ...
        if "discord" in low and "connected" in low:
            ...
            title, icon, accent = "Website Created", "⌂", "#ff1a1a"
            ...
            title, icon, accent = "File Organization Completed", "🗂", "#ff1a1a"
    ...
    def __init__(self, parent=None):
        ...
        self.setStyleSheet(
            ...
            QFrame#TaskDock {{
                ...
                border-left: 1px solid rgba(200, 0, 0, 0.55);
            }}
            """
        )
...
class WorkspaceSidebar(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        header.addWidget(self._title)
        header.addStretch()
        self._close_btn = QPushButton("SATAN")
        ...
        if active:
            return """
                QPushButton {
                    ...
                    color: #FFFFFF;
                    border: 1px solid rgba(200, 0, 0,180);
                    border-radius: 10px;
                    padding: 0 14px;
                }
            """
    ...
    def _build_history_tab(self) -> QWidget:
        ...
        self._history_search.setStyleSheet(
            ...
            QLineEdit {
                ...
                color: #FFFFFF;
                border: 1px solid rgba(200, 0, 0,100);
                border-radius: 12px;
                padding: 0 12px;
            }
            QLineEdit:focus {
                border: 1px solid rgba(200, 0, 0,190);
            }
            """
        )
    ...
    def record_chat_event(self, event: object):
        ...
        if role == "user":
            ...
            self._active_conversation_id = convo_id
            self._feed.add_message("assistant", "SATAN", text, _fmt_time_stamp(stamp), attachments=attachments, animate=True)
            self._hide_memory_banner()
        elif role == "system":
...
class InlineChatWorkspace(QFrame):
    ...
    def _set_tab(self, index: int):
        ...
        if index == 0:
            self._chat_btn.setStyleSheet("""
                QPushButton { background: rgba(200, 0, 0,0.16); color: #FFFFFF; border: 1px solid rgba(200, 0, 0,180); border-radius: 10px; padding: 0 14px; }
            """)
            self._history_btn.setStyleSheet("""
                ...
                QPushButton:hover { background: rgba(200, 0, 0,0.08); border: 1px solid rgba(200, 0, 0,0.30); }
            """)
        else:
            self._history_btn.setStyleSheet("""
                QPushButton { background: rgba(200, 0, 0,0.16); color: #FFFFFF; border: 1px solid rgba(200, 0, 0,180); border-radius: 10px; padding: 0 14px; }
    ...
    def _build_chat_tab(self) -> QWidget:
        ...
        self._footer_status = QLabel("SATAN is listening...")
        ...
        waveform.setStyleSheet("color: #cc0000; background: transparent; letter-spacing: 2px;")
    ...
    def _build_history_tab(self) -> QWidget:
        ...
        self._search.setStyleSheet("QLineEdit { background: rgba(10,11,14,205); color: #FFFFFF; border: 1px solid rgba(200, 0, 0,100); border-radius: 12px; padding: 0 12px; }")
    ...
    def record_chat_event(self, event: object):
        ...
        if role == "user":
            ...
            self._feed.add_message("assistant", "SATAN", text, stamp, attachments=attachments)
            self._hide_memories()
        elif role == "system":
...
class LogWidget(QScrollArea):
    ...
    def _parse(self, text: str) -> tuple[str, str, str]:
        ...
        if tl.startswith("satan:"):
            return "assistant", "SATAN", raw[len("SATAN:"):].strip()
        if tl.startswith("satan:"):
            return "assistant", "SATAN", raw[len("SATAN:"):].strip()
...
class FileDropZone(QWidget):
    ...
    def _browse(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select a file for SATAN", str(Path.home()),
            "All Files (*.*);;"
...
class SetupOverlay(QWidget):
    ...
    def _build_stage1(self):
        ...
        self._s1_lbl.setStyleSheet("color: rgba(200, 0, 0, 0.9); background: transparent; border: none;")
    ...
    def _start_stage1(self):
        self._s1_lines = [
            ("Scanning local configuration...", "#cc0000", False),
            ("✓  " + self._detected.capitalize() + " detected", "#ff1a1a", False),
            ("✓  GPU acceleration enabled", "#ff1a1a", False),
            ("✓  Network online", "#ff1a1a", False),
            ("Looking for AI provider...", "#cc0000", False),
    ...
    def _build_stage2(self):
        ...
        gt.setStyleSheet("color: #cc0000; background: transparent; border: none;")
        glay.addWidget(gt)

        gs = QLabel("★★★★★  Recommended")
        gs.setFont(QFont("Segoe UI", 9))
        gs.setStyleSheet("color: rgba(200,0,0,0.7); background: transparent; border: none;")
        glay.addWidget(gs)
        ...
        gc.setStyleSheet("color: #cc0000; background: transparent; border: none;")
        glay.addWidget(gc)
    ...
    def _build_stage3(self):
        ...
        self._s3_title.setStyleSheet("color: #cc0000; background: transparent; border: none;")
        ...
        self._key_input.setStyleSheet("""
            QLineEdit {
                ...
                padding: 0 16px;
                letter-spacing: 1px;
                selection-background-color: rgba(200, 0, 0, 0.3);
            }
            QLineEdit:focus {
        ...
        toggle_pw.setStyleSheet("QPushButton { background: transparent; border: none; color: rgba(255,255,255,0.3); font-size: 16px; } QPushButton:hover { color: #cc0000; }")
        def _toggle():
        ...
        self._s3_status.setStyleSheet("color: #cc0000; background: transparent; border: none;")
        status_row.addWidget(self._s3_status)

        status_row.addStretch()

        hint = QLabel("<a href='https://aistudio.google.com/app/apikey' style='color: rgba(200,0,0,0.5); text-decoration: none; font-size: 10px;'>Get API Key →</a>")
        ...
        self._intro_lines = []
        for txt in ["Identity confirmed.", "Hello.", "I'm SATAN.", "Ready whenever you are."]:
            lbl = QLabel(txt)
            lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
            if txt == "I'm SATAN.":
                lbl.setFont(QFont("Segoe UI", 24, QFont.Weight.Bold))
                lbl.setStyleSheet("color: #cc0000; background: transparent; border: none;")
            else:
        ...
        self._launch_btn = QPushButton("Launch SATAN →")
    ...
    def _on_key_changed(self, text):
        if len(text) > 10 and not hasattr(self, "_authenticating"):
            ...
            self._s3_sub.setStyleSheet("color: #cc0000; background: transparent; border: none;")
            self._auth_step = 0
    ...
    def _auth_tick(self):
        ...
        if self._auth_step < len(bars):
            ...
            self._s3_status.setStyleSheet("color: #ff1a1a; background: transparent; border: none;")
            self._s3_title.setText("✓ Google Gemini")
            self._s3_title.setStyleSheet("color: #ff1a1a; background: transparent; border: none;")
    ...
    def _build_stage4(self):
        ...
        self._s4_title.setStyleSheet("color: rgba(200,0,0,0.7); background: transparent; border: none; letter-spacing: 3px;")
        ...
        for mod in ["MEMORY", "VOICE", "VISION", "AUTOMATION", "REASONING"]:
            ...
            bar_lbl.setStyleSheet("color: rgba(200,0,0,0.15); background: transparent; border: none;")
    ...
    def _ignite_tick(self):
        if self._ignite_step < len(self._module_order):
            ...
            name_lbl.setStyleSheet("color: #cc0000; background: transparent; border: none; font-weight: bold;")
            bar_lbl.setText("██████████")
            bar_lbl.setStyleSheet("color: #cc0000; background: transparent; border: none;")
            ...
            self._s4_title.setStyleSheet("color: #ff1a1a; background: transparent; border: none; letter-spacing: 3px; font-weight: bold;")
            self._s4_container.setStyleSheet("""
                QFrame {
                    background: rgba(5, 8, 12, 180);
                    border: 1px solid rgba(55, 255, 95, 0.2);
            ...
            for mod in self._module_order:
                ...
                n.setStyleSheet("color: #ff1a1a; background: transparent; border: none; font-weight: bold;")
                b.setStyleSheet("color: #ff1a1a; background: transparent; border: none;")
...
class CommandBar(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        logo_lbl.setStyleSheet("color: #cc0000; background: transparent; border: none;")
        logo_lay.addWidget(logo_lbl)
        lay.addWidget(logo_frame)

        # Input field
        self._input = QLineEdit()
        self._input.setPlaceholderText("Tell SATAN what to do...")
        ...
        self._input.setStyleSheet(f"""
            QLineEdit {{
                ...
                border-radius: 19px;
                padding: 0 18px;
                selection-background-color: rgba(200, 0, 0, 0.25);
            }}
            QLineEdit:focus {{
                border: 1px solid rgba(200, 0, 0, 0.4);
...
class DeveloperModeDialog(QDialog):
    def __init__(self, parent=None, settings: dict | None = None):
        ...
        desc = QLabel("Pick a workspace folder SATAN should use when building websites or other workspace-based tasks.")
...
class MeetingOverlay(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._answer = QLabel("SATAN will show the live answer here.")
...
class FloatingLauncher(QWidget):
    ...
    def paintEvent(self, event):
        ...
        painter.drawText(QRectF(cx - 26, cy - 10, 52, 20), Qt.AlignmentFlag.AlignCenter, "SATAN")
...
class MainWindow(QMainWindow):
    ...
    def _startup_enabled(self) -> bool:
        ...
        try:
            with winreg.OpenKey(
                ...
                try:
                    value, _ = winreg.QueryValueEx(key, "SATAN")
                    run_value = _startup_run_value()
                    if value != run_value:
                        winreg.SetValueEx(key, "SATAN", 0, winreg.REG_SZ, run_value)
                    return bool(value)
    ...
    def set_SATAN_connect_service(self, service):
        self._SATAN_connect = service
    ...
    def _build_header(self) -> QWidget:
        ...
        lay.addWidget(_badge("SATAN", C.PRI_DIM))
        lay.addStretch()
    ...
    def _tick_clock(self):
        ...
        if hasattr(self, "_core_status_lbl") and self._core_status_lbl is not None:
            self._core_status_lbl.setText("SATAN is ready. Gemini 2.5 Flash · OpenRouter · Voice Connected · Memory Enabled")
    ...
    def _browse_attachment(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Attach a file to SATAN", str(Path.home()),
            "All Files (*.*);;"
            "Images (*.jpg *.jpeg *.png *.gif *.webp *.bmp *.svg);;"
            "Documents (*.pdf *.docx *.txt *.md *.pptx);;"
            "Data (*.csv *.xlsx *.json *.xml);;"
        ...
        if hasattr(self, "_result_card") and low.startswith("you:"):
            ...
            if self.on_chat_event and user_msg:
                try:
                    ...
                    pass
        if hasattr(self, "_result_card") and low.startswith("satan:"):
    ...
    def notify_phone_connected(self):
        ...
        self._log_sig.emit("SYS: Phone connected to SATAN remote.")
    ...
    def _apply_state(self, state: str):
        ...
        if hasattr(self, "_task_card"):
            if state == "THINKING":
                self._task_card.set_task("Working on it...", "SATAN is processing your request.", 72)
            elif state == "SPEAKING":
                self._task_card.set_task("Responding...", "SATAN is speaking now.", 100)
            elif state == "MUTED":
                self._task_card.set_task("Microphone muted", "Voice input is paused.", 0)
            else:
                self._task_card.set_task("Ready", "SATAN is idle and ready.", 0)
        if hasattr(self, "_result_card"):
            if state == "THINKING":
    ...
    def _on_setup_done(self, key: str, or_key: str, os_name: str):
        try:
            ...
            if self._overlay:
                ...
                self._overlay = None
            self._apply_state("LISTENING")
            self._log.append_log(f"SYS: Initialised. OS={os_name.upper()}. SATAN online.")
        except Exception as e:
    ...
    def _build_left_panel_modern(self) -> QWidget:
        ...
        class NavItem(QFrame):
            ...
            def set_active(self, active: bool):
                ...
                border_left = "2px solid #cc0000" if self._active else "2px solid transparent"
                self.setStyleSheet(
                    f"""
                    QFrame#LeftNavItem {{
                        background: {bg_style};
                        border: 1px solid rgba(200, 0, 0, { '0.15' if self._active else '0.0' });
                ...
                if self._active:
                    self._icon.setStyleSheet("background: rgba(200, 0, 0, 0.25); color: #cc0000; border-radius: 8px; border: 1px solid #cc0000;")
                    self._lbl.setStyleSheet("color: #ffffff; background: transparent; font-weight: bold;")
                    self._arrow.setStyleSheet("color: #cc0000; background: transparent; font-weight: bold;")
                else:
            ...
            def enterEvent(self, event):
                super().enterEvent(event)
                if not self._active:
                    self._icon.setStyleSheet("background: rgba(200, 0, 0, 0.15); color: #cc0000; border-radius: 8px; border: 1px solid rgba(200, 0, 0, 0.4);")
        ...
        title = QLabel("<span style='color:#cc0000;'>SATAN</span><br><span style='color:#ffffff;'>LITE</span>")
        title.setFont(QFont("Segoe UI", 16, QFont.Weight.Bold))
        ...
        self._nav_items["home"] = NavItem("SATAN Home", active=False, letter="H")
        ...
        online = QLabel("<span style='color:#cc0000;'>●</span> <span style='color:#ffffff; font-weight:700;'>System Online</span>")
        ...
        self._bar_cpu = MetricBar("CPU", "#cc0000")
        self._bar_mem = MetricBar("RAM", "#cc0000")
    ...
    def _build_center_panel_modern(self, face_path: str) -> QWidget:
        ...
        self._devices_page = SATANConnectDevicesPage(self)
    ...
    def _build_command_row(self) -> QHBoxLayout:
        ...
        self._input = QLineEdit()
        self._input.setPlaceholderText("Ask SATAN anything...")
        self._input.setFont(QFont("Segoe UI", 10))
        ...
        mic.setStyleSheet(f"""
            ...
            QPushButton:hover {{
                background: rgba(200, 0, 0, 0.25);
                border: 1px solid rgba(200, 0, 0, 0.60);
            }}
            QPushButton:pressed {{
                background: rgba(200, 0, 0, 0.40);
            }}
        """)
...
class SystemConnectivitySidebar(QFrame):
    def __init__(self, controller=None, parent=None):
        ...
        self._mk_quick_action("↻ Restart SATAN", QStyle.StandardPixmap.SP_BrowserReload, self._restart)
...
class SystemConnectivityPage(QWidget):
    ...
    def _provider_row(self, name: str, key: str, model: str, setting_key: str):
        ...
        if key:
            ...
            row.setStyleSheet("QFrame { background: rgba(255, 255, 255, 0.02); border: 1px solid rgba(255, 255, 255, 0.06); border-radius: 14px; } QFrame:hover { background: rgba(200, 0, 0, 0.04); border: 1px solid rgba(200, 0, 0, 0.3); }")
        r = QHBoxLayout(row)
        ...
        if key:
            ...
            icon.setStyleSheet(f"background: rgba(200, 0, 0, 0.12); color: {C.WHITE}; border: 1px solid rgba(200, 0, 0, 0.38); border-radius: 21px;")
        r.addWidget(icon)
        meta = QVBoxLayout()
        title = QLabel(name)
        title.setFont(QFont("Segoe UI", 11, QFont.Weight.Bold))
    ...
    def _build_left_column(self):
        ...
        # Mobile connect
        mobile = self._card("Mobile Connect", "Connect your phone and control SATAN remotely.")
        ml = mobile.layout()
        ...
        # Startup
        startup = self._card("Startup", "Use SATAN with Windows startup preferences.")
        sl = startup.layout()
        self._startup_launch_btn = self._mk_toggle("Launch SATAN when Windows starts", bool(self._load_app_settings().get("show_workspace_on_startup", False)), self._toggle_startup_from_page)
        ...
        # Shortcuts & Pinning
        shortcuts = self._card("Shortcuts & Pinning", "Create shortcuts and pin SATAN to your Windows system.")
        ...
        self._preview_progress.setStyleSheet("QProgressBar { background: rgba(255,255,255,0.05); border: none; border-radius: 4px; } QProgressBar::chunk { background: #cc0000; border-radius: 4px; }")
        al.addWidget(self._preview_progress)
        lay.addWidget(anim)

        # Discord bot
        discord = self._card("Discord Bot", "Mirror SATAN between the app and your server.")
        dl = discord.layout()
        ...
        about = self._card("About SATAN", "SATAN information only.")
        ab = about.layout()
    ...
    def _build_quick_actions_box(self):
        ...
        lay = box.layout()
        actions = [
            ("Restart SATAN", QStyle.StandardPixmap.SP_BrowserReload, self._restart_app),
    ...
    def _test_discord_from_page(self):
        ...
        if self._ctrl() and hasattr(self._ctrl(), "_win"):
            ...
            self._discord_msg.setText("Connected as SATAN#9649" if self._discord_token.text().strip() else "Bot Offline")
    ...
    def refresh(self):
        ...
        if enabled and token:
            ...
            self._discord_msg.setText("Connected as SATAN#9649")
        elif token:
    ...
    def _create_desktop_shortcut_logic(self):
        try:
            ...
            shortcut_path = desktop_dir / "SATAN - Premium.lnk"
            ...
            ps1_script = "\n".join([
                ...
                "$Shortcut.Description = 'Launch SATAN - Premium'",
    ...
    def _pin_app_to_taskbar_logic(self):
        try:
            ...
            if res.returncode == 0:
                return True, "SATAN has been pinned to your Taskbar!"
            else:
                return False, "Windows restricts programmatic taskbar pinning. Please right-click the 'SATAN - Premium.lnk' shortcut on your Desktop and select 'Pin to taskbar', or drag it directly onto your taskbar."
        except Exception as e:
...
class SmartDevicesSection(QFrame):
    def __init__(self, controller=None, parent=None):
        ...
        empty_btn = QPushButton("Open SATAN Home")
    ...
    def _pick_color(self, device: dict[str, object]):
        color = QColorDialog.getColor(QColor("#cc0000"), self, "Pick Light Color")
        if color.isValid():
...
class SatanUI:
    def __init__(self, face_path: str, size=None, *, show_immediately: bool = True):
        ...
        self._app.setApplicationDisplayName("SATAN")
    ...
    def set_SATAN_connect_service(self, service):
        self._win.set_SATAN_connect_service(service)
...
class SATANConnectDevicesPage(QFrame):
    def __init__(self, parent=None):
        super().__init__(parent)
        self._service = None
        ...
        self.setObjectName("SATANConnectDevicesPage")
        self.setStyleSheet(f"""
            QFrame#SATANConnectDevicesPage {{
                background: transparent;
                border: none;
            }}
            QLabel {{
        ...
        self._subtitle = QLabel("Everything connected to SATAN.")
        ...
        self._add_btn.setStyleSheet("""
            ...
            QPushButton:hover {
                background: rgba(200, 0, 0, 0.16);
                border: 1px solid rgba(200, 0, 0, 0.40);
            }
        """)
        ...
        self._detail_icon.setStyleSheet("background: rgba(200, 0, 0, 0.10); color: #cc0000; border: 1px solid rgba(200,0,0,0.22); border-radius: 12px;")
        ...
        for i, b in enumerate([self._btn_rename, self._btn_disconnect, self._btn_reconnect, self._btn_forget]):
            ...
            if b != self._btn_forget:
                b.setStyleSheet("""
                    ...
                    QPushButton:hover { background: rgba(200,0,0,0.08); border: 1px solid rgba(200,0,0,0.22); color: #cc0000; }
                """)
            else:
                b.setStyleSheet("""
        ...
        self._cs_android_btn.setStyleSheet("""
            ...
            QPushButton:hover { color: #cc0000; border: 1px solid rgba(200,0,0,0.6); background: rgba(200,0,0,0.1); }
        """)
        ...
        self._onb_code_lbl.setStyleSheet("color: #cc0000; letter-spacing: 6px;")
    ...
    def _service_obj(self):
        return self._service or getattr(self.parentWidget(), "_SATAN_connect", None)
    ...
    def _refresh_onboarding_offer(self):
        ...
        try:
            ...
            self._onboarding_offer = dict(service.create_pairing_offer(device_name="SATAN Connect", platform="gateway"))
