...
def _default_config_path(base_dir: Path) -> Path:
    return Path(base_dir) / "config" / "SATAN_connect.json"


def _default_registry_path(base_dir: Path) -> Path:
    return Path(base_dir) / "config" / "SATAN_connect" / "devices.json"


@dataclass(slots=True)
class SATANGatewayConfig:
    host: str = "0.0.0.0"
    port: int = 8765
    enabled: bool = True
    advertise: bool = True
    service_name: str = "_SATAN._tcp.local."
    ...
    @classmethod
    def load(cls, base_dir: Path, override: dict[str, Any] | None = None) -> "SATANGatewayConfig":
        ...
        data: dict[str, Any] = {
            ...
            "enabled": True,
            "advertise": True,
            "service_name": "_SATAN._tcp.local.",
        ...
        return cls(
            ...
            service_name=str(data.get("service_name", "_SATAN._tcp.local.")),
...
class SATANGateway:
    def __init__(self, base_dir: Path, config: SATANGatewayConfig | None = None):
        self.base_dir = Path(base_dir)
        self.config = config or SATANGatewayConfig.load(self.base_dir)
    ...
    async def disconnect_device(self, device_or_target: str, *, reason: str = "Disconnected by SATAN") -> dict[str, Any]:
        query = str(device_or_target or "").strip()
        matches = self.device_manager.resolve(query)
        record = None
                    ...
                    if msg_type == ProtocolTypes.HELLO:
                        ...
                        await websocket.send_json(
                            build_message(
                                ...
                                {
                                    "pending_id": pending_id,
                                    "message": "Pairing request received. Awaiting user approval in SATAN.",
    ...
    async def serve(self) -> None:
        ...
        if self.config.advertise:
            advertised = self.discovery.start(host=self.config.host, port=self.config.port, properties={"service": "SATAN", "version": "1"})
