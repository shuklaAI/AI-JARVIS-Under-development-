...
from .gateway.server import SATANGateway, SATANGatewayConfig


@dataclass(slots=True)
class SATANConnectService:
    """Small runtime wrapper around the gateway server."""

    base_dir: Path
    config: SATANGatewayConfig | None = None
    gateway: SATANGateway = field(init=False)
    _thread: threading.Thread | None = field(init=False, default=None)
    _started: bool = field(init=False, default=False)

    def __post_init__(self) -> None:
        self.base_dir = Path(self.base_dir)
        self.gateway = SATANGateway(self.base_dir, self.config)
    ...
    def start_background(self) -> None:
        ...
        self._thread = threading.Thread(target=_runner, name="SATANConnectGateway", daemon=True)
        self._thread.start()
        self._started = True
    ...
    async def disconnect_device(self, target: str, *, reason: str = "Disconnected by SATAN") -> dict[str, Any]:
...
_SERVICE: SATANConnectService | None = None


def get_service(base_dir: str | Path) -> SATANConnectService:
    global _SERVICE
    if _SERVICE is None:
        _SERVICE = SATANConnectService(Path(base_dir))
    return _SERVICE
