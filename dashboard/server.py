"""
dashboard/server.py — SATAN Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "SATAN Uploads",
        Path.home() / "Documents" / "SATAN Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
# ── AES-256-CBC ───────────────────────────────────────────────────────────────
_AES_SALT = b'SATAN-DASHBOARD-v1'
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"SATAN Dashboard Port {port}"
        prog_rule  = "SATAN Dashboard Python"
        ...
        bat_body = "\r\n".join(bat_lines) + "\r\n"
        fd, bat_path = tempfile.mkstemp(suffix=".bat", prefix="satan_fw_")
        try:
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until SATAN is run as Administrator.")
...
def _ensure_ssl_certs() -> bool:
    ...
    key_path = certs / "satan.key"
    cert_path = certs / "satan.crt"
    ...
    try:
        ...
        subject = issuer = x509.Name([
            x509.NameAttribute(NameOID.COMMON_NAME, "SATAN AI Local Remote"),
        ])
        alt_names = [
...
<p>Press <strong style="color:#dde3ed">Mobile Connect</strong> in SATAN to get a new QR code.</p>
</div></body></html>""")
...
<body>
<script>
  sessionStorage.setItem('satan_token','{tok}');
  sessionStorage.setItem('satan_key','{key}');
  localStorage.setItem('satan_device_token','{dev_tok}');
  setTimeout(function(){{location.replace('/')}},400);
</script>
<p>Connecting to SATAN…</p>
</body></html>""")
    ...
    async def _serve_alias(self) -> None:
        ...
        ssl_key  = BASE_DIR / "config" / "certs" / "satan.key"
        ssl_cert = BASE_DIR / "config" / "certs" / "satan.crt"
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Mobile Connect' in SATAN UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
