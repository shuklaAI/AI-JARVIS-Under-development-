<div align="center">
  <img src="assets/SATAN_Lite_Logo.png" alt="SATAN Echo" width="260" />
...
SATAN Echo is a premium Windows desktop assistant that combines voice and text control with automated workflows, screen-aware intelligence, and rich content generation.

Designed for advanced desktop productivity, SATAN Echo delivers:
...
- Remote control via Discord and SATAN Connect
...
- Wake-word support for “SATAN Echo” and responsive assistant activation
...
- Device pairing and remote routing through SATAN Connect
...
- SATAN Connect for device discovery and command routing
...
```powershell
git clone https://github.com/titechprabhasolutions/SATAN-AI---Lite.git
cd "SATAN AI - Lite"
```
...
- `config/SATAN_connect.json` — device pairing, gateway, and discovery settings
...
- `SATAN_connect/` — local gateway, pairing, and remote routing
...
Extend SATAN Echo with custom Python plugins by adding files to `plugins/`.

Supported hooks:

- `on_SATAN_created(SATAN)` — called when the assistant instance is initialized
- `on_startup(SATAN)` — called after startup when plugins are registered
- `on_text_command(text, source, SATAN=None)` — called for each incoming text command; return `True` to indicate the command was handled
