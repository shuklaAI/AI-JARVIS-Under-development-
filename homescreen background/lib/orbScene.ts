...
export function createOrbScene(container: HTMLElement): OrbSceneApi {
  ...
  const chromaticShader = {
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        // Push towards a deeper red tone
        gl_FragColor.rgb = mix(gl_FragColor.rgb, gl_FragColor.rgb * vec3(1.3, 0.6, 0.6), 0.35);
      }
    `,
  };
  ...
  function makeTextSprite(text: string, size = 0.08) {
    ...
    ctx.fillStyle = `rgba(255, ${(40 + Math.random() * 40) | 0}, ${(40 + Math.random() * 40) | 0}, ${alpha})`;
  ...
  g.addColorStop(0.2, "rgba(220,50,50,0.6)");
  g.addColorStop(0.5, "rgba(140,40,40,0.15)");
