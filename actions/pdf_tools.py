"""
pdf_tools.py - SATAN AI PDF support
...
def _pdf_story_from_blocks(blocks: list[dict], pdf, styles):
    ...
    for block in blocks:
        ...
        if kind == "heading":
            ...
            story.append(pdf["Paragraph"](text, styles.get(f"SATAN_h{min(max(level, 1), 3)}", styles["SATAN_h1"])))
            story.append(pdf["Spacer"](1, 0.1 * pdf["inch"]))
        elif kind == "bullet":
            story.append(pdf["Paragraph"](f"- {text}", styles["SATAN_body"]))
            story.append(pdf["Spacer"](1, 0.03 * pdf["inch"]))
        elif kind == "numbered":
            story.append(pdf["Paragraph"](f"{number_index}. {text}", styles["SATAN_body"]))
            number_index += 1
            story.append(pdf["Spacer"](1, 0.06 * pdf["inch"]))
        elif kind == "table":
            rows = block.get("text") or []
            table_data = [[pdf["Paragraph"](cell, styles["SATAN_table_cell"]) for cell in row.split(" | ")] for row in rows]
            if table_data:
            ...
            story.append(pdf["Paragraph"](text, styles["SATAN_body"]))
...
def create_pdf(parameters: dict, player=None) -> str:
    ...
    output_path = _resolve_output_path(parameters.get("output_path"), title, ".pdf", "SATAN_ai_output")
    ...
    if title or subtitle:
        ...
        story.append(pdf["Paragraph"]("<hr/>", styles["SATAN_body"]))
    ...
    if action == "create_letter":
        ...
        if recipient:
            story.append(pdf["Paragraph"](recipient, styles["SATAN_body"]))
            story.append(pdf["Spacer"](1, 0.06 * pdf["inch"]))
        story.append(pdf["Paragraph"](salutation, styles["SATAN_body"]))
        story.append(pdf["Spacer"](1, 0.08 * pdf["inch"]))
        for para in _normalize_list(parameters.get("paragraphs")) or [p.strip() for p in re.split(r"\n\s*\n", str(body_text)) if p.strip()]:
            story.append(pdf["Paragraph"](str(para), styles["SATAN_body"]))
            story.append(pdf["Spacer"](1, 0.08 * pdf["inch"]))
        story.append(pdf["Spacer"](1, 0.18 * pdf["inch"]))
        story.append(pdf["Paragraph"](closing, styles["SATAN_body"]))
        story.append(pdf["Spacer"](1, 0.3 * pdf["inch"]))
        story.append(pdf["Paragraph"](signature, styles["SATAN_body"]))
    else:
