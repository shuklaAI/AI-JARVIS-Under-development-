"""
docx_tools.py - SATAN AI Word / DOCX support
...
def _docx_result_path(source_path: Path | None, action: str, output_path: str | None, title: str) -> Path:
    if output_path:
        fallback = title or (source_path.stem if source_path else "SATAN_AI_Document")
    ...
    return _resolve_output_path(None, title=title, ext=".docx", fallback_name="SATAN_AI_Document")
...
def word_document(parameters: dict, player=None, speak=None) -> str:
    ...
    title = (params.get("title") or params.get("subject") or "SATAN AI Document").strip()
