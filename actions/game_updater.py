...
def _find_steam_path() -> Path | None:
    # Steam registry lookup only works on Windows
    if sys.platform != "win32":
        return None
...
def _cancel_scheduled_update() -> str:
    result = subprocess.run(["schtasks", "/Delete", "/TN", "SATANAI_GameUpdater", "/F"],
                            capture_output=True, text=True)
    return "Scheduled update cancelled." if result.returncode == 0 else "No scheduled update found."


def _get_schedule_status() -> str:
    result = subprocess.run(["schtasks", "/Query", "/TN", "SATANAI_GameUpdater", "/FO", "LIST"],
                            capture_output=True, text=True)
    if result.returncode != 0:
