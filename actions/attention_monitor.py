...
def speak_native(text: str) -> None:
    ...
    audio_path = os.path.join(tempfile.gettempdir(), f"SATAN_edge_tts_{uuid.uuid4().hex}.mp3")
    try:
        # Use a male neural voice for app speech so daily briefing and alerts sound
        # closer to SATAN's normal male audio output.
    ...
    player_alias = f"SATAN_tts_{uuid.uuid4().hex}"
    try:
