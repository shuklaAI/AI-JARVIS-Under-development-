...
new_setup_overlay = """class SetupOverlay(QWidget):
    ...
    def _build_stage3(self):
        ...
        self._key_input.setStyleSheet(f\"\"\"
            QLineEdit {{
                background: rgba(255, 50, 50, 0.05); color: #ff3333;
                border: 1px solid rgba(255, 50, 50, 0.4); border-radius: 12px; padding: 0 16px;
                letter-spacing: 2px;
            }}
        ...
        self._s3_status.setStyleSheet(f"color: #ff3333; background: transparent; border: none;")
    ...
    def _auth_tick(self):
        ...
        if self._auth_step < len(bars):
            ...
            self._key_input.setStyleSheet(self._key_input.styleSheet().replace("rgba(255, 50, 50, 0.4)", C.GREEN).replace("#ff3333", C.GREEN))
            self._s3_title.setStyleSheet(f"color: {C.GREEN}; background: transparent; border: none;")
            
            QTimer.singleShot(1500, self._intro_SATAN)
            
    def _intro_SATAN(self):
        self._s3_box.hide()
    ...
    def _build_stage4(self):
        ...
        self._s4_title.setStyleSheet("color: #ff3333; background: transparent; border: none;")
