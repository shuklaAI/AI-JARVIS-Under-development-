...
new_setup_overlay = """class SetupOverlay(QWidget):
    ...
    def _build_step1(self):
        ...
        lay.addWidget(self._header("SATAN ECHO", "First Time Setup\\nConnect your preferred AI provider to begin."))
        lay.addStretch()
    ...
    def _build_step2(self):
        ...
        self._key_input.setStyleSheet(f\"\"\"
            ...
            QLineEdit:focus {{ border: 1px solid rgba(255, 50, 50, 0.4); }}
        \"\"\")
        self._key_input.setText((self._defaults.get("gemini_api_key") or "").strip())
        lay.addWidget(self._key_input)
        
        hint_lay = QHBoxLayout()
        hint = QLabel("Need one? <a href='https://aistudio.google.com/app/apikey' style='color:#ff3333; text-decoration:none;'>Get API Key →</a>")
    ...
    def _sel(self, key: str):
        ...
        for k, btn in self._os_btns.items():
            if k == key:
                btn.setStyleSheet(f\"\"\"
                    QPushButton {{
                        background: rgba(255, 50, 50, 0.12); color: #ff3333;
                        border: 1px solid rgba(255, 50, 50, 0.4); border-radius: 10px; font-weight: bold;
                    }}
                \"\"\")
            else:
                btn.setStyleSheet(f\"\"\"
    ...
    def _build_step_boot(self):
        ...
        self._boot_lbl.setStyleSheet(f"color: #ff3333; background: transparent; border: none;")
...
# Update _show_setup in SATANUI to make overlay full screen
