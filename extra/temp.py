...
from smart_home_page_new import SATANHomePage, _DeviceTile
...
LOGO_FILE  = BASE_DIR / "assets" / "SATAN_Lite_Logo.png"
LOGO_ICO   = BASE_DIR / "assets" / "SATAN_Lite_Logo.ico"
...
class C:
    ...
    BORDER_B  = "rgba(255,50,50, 0.20)"
    BORDER_A  = "rgba(255,50,50, 0.15)"
    PRI       = "#ff3333"
    PRI_DIM   = "#e59400"
    PRI_GHO   = "rgba(255,50,50, 0.15)"
    ACC       = "#ff3333"
    ACC2      = "#ff9999"
    GREEN     = "#37ff5f"
    GREEN_D   = "#1dcc43"
    ...
    def set_ai_state(self, state: str) -> None:
        if self._web_view:
            try:
                ...
                self._web_view.page().runJavaScript(f"if(window.setSATANState) window.setSATANState('{st}');")
            except Exception:
                pass
...
class RemoteKeyOverlay(QWidget):
    ...
    def __init__(self, url: str, key: str, auto: str, manual: str, parent=None):
        ...
        subtitle = QLabel("Scan the QR code with your phone to remotely control SATAN Echo.")
    ...
    def mark_connected(self) -> None:
        ...
        self._timer_lbl.setText("Phone connected. SATAN Echo remote is ready.")
...
class _SysMetrics:
    ...
    def _get_gpu(self) -> float:
        ...
        # macOS ├óΓé¼ΓÇ¥ powermetrics (GPU Engine)
        if _OS == "Darwin":
            try:
                r = _quiet_run(
...
class GestureCameraPreview(QFrame):
    def __init__(self, parent=None):
        ...
        self._sensitivity_select.setStyleSheet(
            "QComboBox { background: rgba(255,255,255,0.05); color: #f4f6f8; border: 1px solid rgba(255,50,50,0.24); border-radius: 8px; padding: 4px 8px; }"
    ...
    def _tick(self):
        ...
        try:
            ...
            if not ret or frame is None:
                self._set_status("Camera feed dropped. Trying againΓÇª", "lost")
                return
...
class TaskCard(QFrame):
    def __init__(self, parent=None):
        ...
        self._plan_lbl = QLabel("Plan: SATAN Echo will generate a task plan after you send a command.")
    ...
    def set_task(self, title: str, desc: str, percent: int):
        ...
        self._plan_lbl.setText("Plan: SATAN Echo will generate a task plan after you send a command.")
    ...
    def _format_plan(self, plan: list[str] | str | None) -> str:
        if not plan:
            return "Plan: ΓÇó Understand the request\n       ΓÇó Execute the right tools\n       ΓÇó Return the result"
        ...
        if not items:
            return "Plan: ΓÇó Understand the request\n       ΓÇó Execute the right tools\n       ΓÇó Return the result"
        return "Plan:\n" + "\n".join(f"ΓÇó {item}" for item in items)
    ...
    def __init__(self, title: str, subtitle: str = "", parent=None):
        ...
        self.setStyleSheet("""
            QFrame#AttachmentCard {
                ...
                border: 1px solid rgba(255,50,50,0.26);
                border-radius: 10px;
            }
        """)
        lay = QHBoxLayout(self)
        ...
        icon.setStyleSheet("color: #ff3333; background: rgba(255,255,255,0.03); border-radius: 13px; font-size: 14px;")
        lay.addWidget(icon)
        txt = QVBoxLayout()
...
class EventCard(QFrame):
    def __init__(self, title: str, detail: str, stamp: str, icon: str = "ΓùÅ", accent: str = "#ff3333", parent=None):
        super().__init__(parent)
...
class ArtifactCard(QFrame):
    def __init__(self, title: str, file_type: str = "File", status: str = "Generated", path: str = "", parent=None):
        ...
        badge.setStyleSheet("background: rgba(255,50,50,0.08); color: #ff3333; border: 1px solid rgba(255,50,50,0.24); border-radius: 15px;")
        ...
        type_lbl = QLabel(f"{file_type} ΓÇó {status}")
...
class ChatBubble(QFrame):
    def __init__(self, role: str, name: str, text: str, stamp: str, attachments: list[dict] | None = None, parent=None, animate: bool = False):
        ...
        if role == "assistant":
            avatar = _framed_logo(24, 24, bg="rgba(12,14,20,245)", border="rgba(255,50,50,0.50)", radius=12, inset=4)
            head.addWidget(avatar)
            name_lbl = QLabel(name or "SATAN Echo")
            name_lbl.setFont(QFont("Segoe UI", 9, QFont.Weight.Bold))
            name_lbl.setStyleSheet("color: #ffffff; background: transparent;")
...
class HistoryConversationItem(QFrame):
    ...
    def __init__(self, conversation_id: str, title: str, stamp: str, pinned: bool = False, parent=None):
        ...
        icon.setAlignment(Qt.AlignmentFlag.AlignCenter)
        icon.setStyleSheet("background: rgba(255,50,50,0.10); color: #ff3333; border: 1px solid rgba(255,50,50,0.28); border-radius: 15px; font: 700 11pt 'Segoe UI';")
        lay.addWidget(icon)
...
class ConversationFeed(QScrollArea):
    ...
    def _ensure_empty_widget(self):
        ...
        lay.setSpacing(10)
        title = QLabel("Try asking SATAN Echo")
    ...
    def _build_event_card(self, text: str, stamp: str, event_type: str | None = None) -> QWidget:
        ...
        if "discord" in low and "connected" in low:
            title, icon, accent = "Discord Connected", "Γùë", "#5865F2"
        elif "presentation" in low:
            title, icon, accent = "Presentation Generated", "Γûú", "#ff7a45"
        elif "website" in low:
            title, icon, accent = "Website Created", "Γîé", "#37ff5f"
        elif "spreadsheet" in low:
            title, icon, accent = "Spreadsheet Generated", "Γûª", "#6fd6ff"
        elif "browser" in low:
            title, icon, accent = "Browser Automation Completed", "Γåù", "#ffbf00"
        elif "organ" in low and "file" in low:
            title, icon, accent = "File Organization Completed", "≡ƒùé", "#37ff5f"
        elif "screen" in low and "analysis" in low:
            title, icon, accent = "Screen Analysis Completed", "Γù½", "#6fd6ff"
    ...
    def __init__(self, parent=None):
        ...
        self.setStyleSheet(
            ...
            QFrame#TaskDock {{
                ...
                border-left: 1px solid rgba(255,50,50, 0.55);
            }}
            """
        )
...
class WorkspaceSidebar(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        header.addWidget(self._title)
        header.addStretch()
        self._close_btn = QPushButton("SATAN ECHO")
        ...
        self._close_btn.setStyleSheet(
            ...
            QPushButton {
                ...
                color: #FFFFFF;
                border: 1px solid rgba(255,50,50,120);
                border-radius: 10px;
                padding: 0 12px;
            }
            QPushButton:hover {
                border: 1px solid rgba(255,50,50,200);
            }
            """
        )
        ...
        if active:
            return """
                QPushButton {
                    ...
                    color: #FFFFFF;
                    border: 1px solid rgba(255,50,50,180);
                    border-radius: 10px;
                    padding: 0 14px;
                }
            """
    ...
    def _build_history_tab(self) -> QWidget:
        ...
        self._history_search.setStyleSheet(
            ...
            QLineEdit:focus {
                border: 1px solid rgba(255,50,50,190);
            }
            """
        )
                ...
                if item["id"] == current:
                    widget.setStyleSheet(
                        ...
                        QFrame#HistoryConversationItem:hover {
                            ...
                            border: 1px solid rgba(255,50,50,0.70);
                        }
                        """
                    )
    ...
    def _show_memory_banner(self, memories: list[dict[str, object]]):
        ...
        if not texts:
            ...
            return
        self._memory_items.setText("ΓÇó " + "\nΓÇó ".join(texts[:4]))
    ...
    def append_log(self, text: str):
        ...
        if not raw:
            return
        low = raw.lower()
        if low.startswith(("you:", "SATAN echo:")):
            return
    ...
    def record_chat_event(self, event: object):
        ...
        if role == "user":
            ...
            self._active_conversation_id = convo_id
            self._feed.add_message("assistant", "SATAN Echo", text, _fmt_time_stamp(stamp), attachments=attachments, animate=True)
            self._hide_memory_banner()
        elif role == "system":
    ...
    def apply_task_workspace(self, event: object):
        ...
        if action == "start":
            ...
            plan_text = plan if isinstance(plan, str) else "\n".join(f"ΓÇó {item}" for item in plan) if plan else ""
...
class InlineChatWorkspace(QFrame):
    ...
    def _set_tab(self, index: int):
        ...
        if index == 0:
            self._chat_btn.setStyleSheet("""
                QPushButton { background: rgba(255,50,50,0.16); color: #FFFFFF; border: 1px solid rgba(255,50,50,180); border-radius: 10px; padding: 0 14px; }
            """)
            self._history_btn.setStyleSheet("""
                ...
                QPushButton:hover { background: rgba(255,50,50,0.08); border: 1px solid rgba(255,50,50,0.30); }
            """)
        else:
            self._history_btn.setStyleSheet("""
                QPushButton { background: rgba(255,50,50,0.16); color: #FFFFFF; border: 1px solid rgba(255,50,50,180); border-radius: 10px; padding: 0 14px; }
    ...
    def _build_chat_tab(self) -> QWidget:
        ...
        self._footer_status = QLabel("SATAN Echo is listening...")
        ...
        waveform = QLabel("πÇ░πÇ░πÇ░")
        waveform.setFont(QFont("Segoe UI", 10, QFont.Weight.Bold))
        waveform.setStyleSheet("color: #ff3333; background: transparent; letter-spacing: 2px;")
    ...
    def _build_history_tab(self) -> QWidget:
        ...
        self._search.setStyleSheet("QLineEdit { background: rgba(10,11,14,205); color: #FFFFFF; border: 1px solid rgba(255,50,50,100); border-radius: 12px; padding: 0 12px; }")
    ...
    def record_chat_event(self, event: object):
        ...
        if role == "user":
            ...
            self._feed.add_message("assistant", "SATAN Echo", text, stamp, attachments=attachments)
            self._hide_memories()
        elif role == "system":
...
class LauncherControlPanel(QDialog):
    def __init__(self, *, startup_workspace: bool = False, on_open=None, on_close=None,
        ...
        title = QLabel("SATAN ECHO CONTROL")
        title.setFont(QFont("Segoe UI", 13, QFont.Weight.Bold))
        title.setStyleSheet("color: #FFFFFF; background: transparent; letter-spacing: 1px;")
        lay.addWidget(title)
        ...
        self._restart_btn = mk_btn("Restart SATAN Echo")
        self._quit_btn = mk_btn("Quit SATAN Echo")
    ...
    def _hide_icon_confirm(self):
        ...
        flay.setSpacing(10)
        lbl = QLabel("Hide SATAN Echo icon?")
...
class LogWidget(QScrollArea):
    ...
    def _parse(self, text: str) -> tuple[str, str, str]:
        ...
        if tl.startswith("SATAN echo:"):
            return "assistant", "SATAN Echo", raw[len("SATAN Echo:"):].strip()
        if tl.startswith("SATAN echo:"):
            return "assistant", "SATAN Echo", raw[len("SATAN Echo:"):].strip()
...
_FILE_ICONS = {
    ...
    "audio":   ("├░┼╕┼╜┬╡", "#cc44ff"), "pdf":     ("├░┼╕ΓÇ£ΓÇ₧", "#ff4444"),
    "word":    ("├░┼╕ΓÇ£┬¥", "#4488ff"), "excel":   ("├░┼╕ΓÇ£┼á", "#44bb44"),
    "code":    ("├░┼╕ΓÇÖ┬╗", "#ffcc00"), "archive": ("├░┼╕ΓÇ£┬ª", "#ff8844"),
    "pptx":    ("├░┼╕ΓÇ£┼á", "#ff6622"), "text":    ("├░┼╕ΓÇ£╞Æ", "#aaaaaa"),
    "data":    ("├░┼╕ΓÇ¥┬º", "#88ddff"), "unknown": ("├░┼╕ΓÇ£┼╜", "#888888"),
}
_EXT_TO_CAT = {
...
class FileDropZone(QWidget):
    ...
    def _browse(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select a file for SATAN Echo", str(Path.home()),
            "All Files (*.*);;"
...
class _DropCanvas(QWidget):
    ...
    def _paint_drag_over(self, p, W, H):
        ...
        p.drawText(QRectF(0, cy - 24, W, 32), Qt.AlignmentFlag.AlignCenter, "Γ¼ç")
    ...
    def _paint_file(self, p, W, H):
        ...
        par = str(path.parent)
        if len(par) > 42: par = "ΓÇª" + par[-41:]
        p.drawText(QRectF(tx, H * 0.18 + 34, tw, 12),
                   Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter, par)

        p.setFont(QFont("Courier New", 9, QFont.Weight.Bold))
        p.setPen(QPen(qcol(C.RED, 180), 1))
        p.drawText(QRectF(W - 34, 0, 28, H), Qt.AlignmentFlag.AlignCenter, "Γ£û")
...
class SetupOverlay(QWidget):
    ...
    # ΓöÇΓöÇ STAGE 1: System Check ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    def _build_stage1(self):
        ...
        self._s1_lbl.setStyleSheet("color: rgba(255,50,50, 0.9); background: transparent; border: none;")
    ...
    def _start_stage1(self):
        self._s1_lines = [
            ("Scanning local configuration...", "#ff3333", False),
            ("Γ£ô  " + self._detected.capitalize() + " detected", "#37ff5f", False),
            ("Γ£ô  GPU acceleration enabled", "#37ff5f", False),
            ("Γ£ô  Network online", "#37ff5f", False),
            ("Looking for AI provider...", "#ff3333", False),
            ("Γ£ò  No provider configured", "#ff3b30", True),
            ("", "", False),
    ...
    def _s1_tick(self):
        if self._s1_idx < len(self._s1_lines):
            ...
            if is_special and "Γ£ò" in line_text:
    ...
    # ΓöÇΓöÇ STAGE 2: Provider Selection ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    def _build_stage2(self):
        ...
        gt.setStyleSheet("color: #ff3333; background: transparent; border: none;")
        glay.addWidget(gt)

        gs = QLabel("ΓÿàΓÿàΓÿàΓÿàΓÿà  Recommended")
        gs.setFont(QFont("Segoe UI", 9))
        gs.setStyleSheet("color: rgba(255,50,50,0.7); background: transparent; border: none;")
        glay.addWidget(gs)
        ...
        gc = QLabel("Connect ΓåÆ")
        gc.setFont(QFont("Segoe UI", 11, QFont.Weight.Bold))
        gc.setStyleSheet("color: #ff3333; background: transparent; border: none;")
        glay.addWidget(gc)
    ...
    # ΓöÇΓöÇ STAGE 3 & 4: API Input + Auth ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    def _build_stage3(self):
        ...
        self._s3_title.setStyleSheet("color: #ff3333; background: transparent; border: none;")
        ...
        self._key_input.setStyleSheet("""
            QLineEdit {
                ...
                padding: 0 16px;
                letter-spacing: 1px;
                selection-background-color: rgba(255,50,50, 0.3);
            }
            QLineEdit:focus {
        ...
        toggle_pw = QPushButton("≡ƒæü")
        toggle_pw.setCursor(Qt.CursorShape.PointingHandCursor)
        toggle_pw.setFixedSize(36, 48)
        toggle_pw.setStyleSheet("QPushButton { background: transparent; border: none; color: rgba(255,255,255,0.3); font-size: 16px; } QPushButton:hover { color: #ff3333; }")
        def _toggle():
        ...
        self._s3_status.setStyleSheet("color: #ff3333; background: transparent; border: none;")
        status_row.addWidget(self._s3_status)

        status_row.addStretch()

        hint = QLabel("<a href='https://aistudio.google.com/app/apikey' style='color: rgba(255,50,50,0.5); text-decoration: none; font-size: 10px;'>Get API Key ΓåÆ</a>")
        ...
        self._intro_lines = []
        for txt in ["Identity confirmed.", "Hello.", "I'm SATAN Echo.", "Ready whenever you are."]:
            lbl = QLabel(txt)
            lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
            if txt == "I'm SATAN Echo.":
                lbl.setFont(QFont("Segoe UI", 24, QFont.Weight.Bold))
                lbl.setStyleSheet("color: #ff3333; background: transparent; border: none;")
            else:
        ...
        self._launch_btn = QPushButton("Launch SATAN Echo ΓåÆ")
    ...
    def _on_key_changed(self, text):
        if len(text) > 10 and not hasattr(self, "_authenticating"):
            ...
            self._s3_sub.setStyleSheet("color: #ff3333; background: transparent; border: none;")
            self._auth_step = 0
    ...
    def _auth_tick(self):
        bars = [
            "ΓûêΓûæΓûæΓûæΓûæΓûæΓûæΓûæΓûæΓûæΓûæΓûæΓûæΓûæΓûæ",
            "ΓûêΓûêΓûêΓûêΓûæΓûæΓûæΓûæΓûæΓûæΓûæΓûæΓûæΓûæΓûæ",
            "ΓûêΓûêΓûêΓûêΓûêΓûêΓûêΓûêΓûæΓûæΓûæΓûæΓûæΓûæΓûæ",
            "ΓûêΓûêΓûêΓûêΓûêΓûêΓûêΓûêΓûêΓûêΓûêΓûæΓûæΓûæΓûæ",
            "ΓûêΓûêΓûêΓûêΓûêΓûêΓûêΓûêΓûêΓûêΓûêΓûêΓûêΓûêΓûê",
        ]
        if self._auth_step < len(bars):
            ...
            self._s3_status.setText("Γ£ô Identity Verified")
            self._s3_status.setStyleSheet("color: #37ff5f; background: transparent; border: none;")
            self._s3_title.setText("Γ£ô Google Gemini")
    ...
    def _show_intro_final(self):
        """Show the SATAN Echo intro sequence."""
        page = self._stack.widget(2)
        lay = page.layout()
    ...
    # ΓöÇΓöÇ STAGE 6: Neural Link Ignition ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    def _build_stage4(self):
        ...
        self._s4_title.setStyleSheet("color: rgba(255,50,50,0.7); background: transparent; border: none; letter-spacing: 3px;")
        ...
        for mod in ["MEMORY", "VOICE", "VISION", "AUTOMATION", "REASONING"]:
            ...
            bar_lbl = QLabel("ΓûæΓûæΓûæΓûæΓûæΓûæΓûæΓûæΓûæΓûæ")
            bar_lbl.setFont(QFont("Consolas", 11))
            bar_lbl.setStyleSheet("color: rgba(255,50,50,0.15); background: transparent; border: none;")
    ...
    def _ignite_tick(self):
        if self._ignite_step < len(self._module_order):
            ...
            name_lbl.setStyleSheet("color: #ff3333; background: transparent; border: none; font-weight: bold;")
            bar_lbl.setText("ΓûêΓûêΓûêΓûêΓûêΓûêΓûêΓûêΓûêΓûê")
            bar_lbl.setStyleSheet("color: #ff3333; background: transparent; border: none;")
...
class CommandBar(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        logo_lbl = QLabel("\u092C\u094D\u0930")  # αñ¼αÑìαñ░ (short Hindi)
        logo_lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
        logo_lbl.setFont(QFont("Nirmala UI", 10, QFont.Weight.Bold))
        logo_lbl.setStyleSheet("color: #ff3333; background: transparent; border: none;")
        logo_lay.addWidget(logo_lbl)
        lay.addWidget(logo_frame)

        # Input field
        self._input = QLineEdit()
        self._input.setPlaceholderText("Tell SATAN Echo what to do...")
        ...
        self._input.setStyleSheet(f"""
            QLineEdit {{
                ...
                border-radius: 19px;
                padding: 0 18px;
                selection-background-color: rgba(255,50,50, 0.25);
            }}
            QLineEdit:focus {{
                border: 1px solid rgba(255,50,50, 0.4);
...
class DeveloperModeDialog(QDialog):
    def __init__(self, parent=None, settings: dict | None = None):
        ...
        desc = QLabel("Pick a workspace folder SATAN Echo should use when building websites or other workspace-based tasks.")
        ...
        try:
            ...
            if self._time >= 1.5:
                ...
                if self._time >= 2.5:
                    ...
                    painter.drawText(QRectF(-ring_rad, -60, ring_rad*2, 60), Qt.AlignmentFlag.AlignCenter, "SATAN")
...
class MeetingOverlay(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._answer = QLabel("SATAN Echo will show the live answer here.")
...
class FloatingLauncher(QWidget):
    ...
    def paintEvent(self, event):
        ...
        # ΓöÇΓöÇ 1. Deep outer ambient glow ΓöÇΓöÇ
        ...
        # ΓöÇΓöÇ 2. Outer dashed orbit ring ΓöÇΓöÇ
        ...
        # ΓöÇΓöÇ 3. Triple rotating arcs ΓöÇΓöÇ
        ...
        # ΓöÇΓöÇ 4. Core dark circle with radial gradient ΓöÇΓöÇ
        ...
        # ΓöÇΓöÇ 5. Particle trails (comet effect) ΓöÇΓöÇ
        ...
        # ΓöÇΓöÇ 6. Hindi SATAN Echo text "αñ¼αÑìαñ░αñ╣αÑìαñ«αñ╛ αñçαñòαÑï" ΓöÇΓöÇ
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        self.setWindowTitle("SATAN Echo")
    ...
    def _startup_enabled(self) -> bool:
        ...
        try:
            with winreg.OpenKey(
                ...
                try:
                    value, _ = winreg.QueryValueEx(key, "SATAN Echo")
                    run_value = _startup_run_value()
                    if value != run_value:
                        winreg.SetValueEx(key, "SATAN Echo", 0, winreg.REG_SZ, run_value)
                    return bool(value)
        ...
        try:
            with winreg.CreateKey(winreg.HKEY_CURRENT_USER, _startup_registry_key()) as key:
                if enabled:
                    ...
                    try:
                        winreg.DeleteValue(key, "SATAN Echo")
                    except FileNotFoundError:
                        pass
            return True
        except Exception as e:
    ...
    def _build_header(self) -> QWidget:
        ...
        lay.addWidget(_badge("SATAN ECHO", C.PRI_DIM))
        lay.addStretch()

        mid = QVBoxLayout(); mid.setSpacing(1)
        title = QLabel("SATAN ECHO")
        title.setAlignment(Qt.AlignmentFlag.AlignCenter)
    ...
    def _tick_clock(self):
        ...
        if hasattr(self, "_date_lbl") and self._date_lbl is not None:
            self._date_lbl.setText(time.strftime("%A ΓÇó %d %b"))
        ...
        if hasattr(self, "_core_status_lbl") and self._core_status_lbl is not None:
            self._core_status_lbl.setText("SATAN Echo is ready. Gemini 2.5 Flash ┬╖ OpenRouter ┬╖ Voice Connected ┬╖ Memory Enabled")
    ...
    def _build_right_panel(self) -> QWidget:
        ...
        self._file_hint = QLabel("No file loaded ├óΓé¼ΓÇ¥ drop or click above to upload")
        ...
        self._mute_btn = QPushButton("├░┼╕┼╜Γäó  MICROPHONE ACTIVE")
        ...
        fs_btn = QPushButton("├óΓÇ║┬╢  FULLSCREEN  [F11]")
    ...
    def _build_input_row(self) -> QHBoxLayout:
        ...
        self._input = QLineEdit()
        self._input.setPlaceholderText("Type a command or question├óΓé¼┬ª")
    ...
    def _build_footer(self) -> QWidget:
        ...
        lay.addWidget(_fl("[F4] Mute  ├é┬╖  [F11] Fullscreen"))
        lay.addStretch()
        lay.addWidget(_fl("Suryaansh Tiwari  ├é┬╖  SATAN Echo  ├é┬╖  Open Source"))
        lay.addStretch()
        lay.addWidget(_fl("├é┬⌐ STARK INDUSTRIES", C.PRI_DIM))
        return w
    ...
    def _on_file_selected(self, path: str):
        ...
        if hasattr(self, "_file_chip") and self._file_chip:
            self._file_chip.setText(f"Attached: {icon} {p.name}  ├óΓé¼┬ó  {size}")
    ...
    def _browse_attachment(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Attach a file to SATAN Echo", str(Path.home()),
            "All Files (*.*);;"
            "Images (*.jpg *.jpeg *.png *.gif *.webp *.bmp *.svg);;"
            "Documents (*.pdf *.docx *.txt *.md *.pptx);;"
            "Data (*.csv *.xlsx *.json *.xml);;"
    ...
    def submit_command(self, txt: str, source: str = "local"):
        ...
        if hasattr(self, "_command_card"):
            preview = txt[:60] + ("ΓÇª" if len(txt) > 60 else "")
        ...
        if hasattr(self, "_result_card") and low.startswith("you:"):
            ...
            if self.on_chat_event and user_msg:
                try:
                    ...
                    pass
        if hasattr(self, "_result_card") and low.startswith("SATAN echo:"):
            reply = raw.split(":", 1)[1].strip()
            self._result_card.set_body(reply[:80] + ("ΓÇª" if len(reply) > 80 else ""))
            self._result_card.hide()
            self._restart_card_hide_timer()
            source = self._chat_source_queue[0] if self._chat_source_queue else "local"
    ...
    def notify_phone_connected(self):
        ...
        self._log_sig.emit("SYS: Phone connected to SATAN Echo remote.")
    ...
    def _apply_state(self, state: str):
        ...
        if hasattr(self, "_status_chip"):
            chip_text = {
                "THINKING": "ΓùÅ WORKING",
                "SPEAKING": "ΓùÅ SPEAKING",
                "MUTED":    "ΓùÅ MUTED",
            }.get(state, "ΓùÅ ONLINE")
        ...
        if hasattr(self, "_task_card"):
            if state == "THINKING":
                self._task_card.set_task("Working on it...", "SATAN Echo is processing your request.", 72)
            elif state == "SPEAKING":
                self._task_card.set_task("Responding...", "SATAN Echo is speaking now.", 100)
            elif state == "MUTED":
                self._task_card.set_task("Microphone muted", "Voice input is paused.", 0)
            else:
                self._task_card.set_task("Ready", "SATAN Echo is idle and ready.", 0)
        if hasattr(self, "_result_card"):
            if state == "THINKING":
    ...
    def _on_setup_done(self, key: str, or_key: str, os_name: str):
        try:
            ...
            if self._overlay:
                ...
                self._overlay = None
            self._apply_state("LISTENING")
            self._log.append_log(f"SYS: Initialised. OS={os_name.upper()}. SATAN Echo online.")
        except Exception as e:
    ...
    def _build_left_panel_modern(self) -> QWidget:
        ...
        class NavItem(QFrame):
            ...
            def set_active(self, active: bool):
                ...
                border_left = "2px solid #ff3333" if self._active else "2px solid transparent"
                self.setStyleSheet(
                    f"""
                    QFrame#LeftNavItem {{
                        background: {bg_style};
                        border: 1px solid rgba(255,50,50, { '0.15' if self._active else '0.0' });
                ...
                if self._active:
                    self._icon.setStyleSheet("background: rgba(255,50,50, 0.25); color: #ff3333; border-radius: 8px; border: 1px solid #ff3333;")
                    self._lbl.setStyleSheet("color: #ffffff; background: transparent; font-weight: bold;")
                    self._arrow.setStyleSheet("color: #ff3333; background: transparent; font-weight: bold;")
                else:
            ...
            def enterEvent(self, event):
                super().enterEvent(event)
                if not self._active:
                    self._icon.setStyleSheet("background: rgba(255,50,50, 0.15); color: #ff3333; border-radius: 8px; border: 1px solid rgba(255,50,50, 0.4);")
        ...
        title = QLabel("<span style='color:#ff3333;'>SATAN ECHO</span><br><span style='color:#ffffff;'>LITE</span>")
        title.setFont(QFont("Segoe UI", 16, QFont.Weight.Bold))
        ...
        self._nav_items["home"] = NavItem("SATAN Echo Home", active=False, letter="H")
        ...
        online = QLabel("<span style='color:#ff3333;'>ΓùÅ</span> <span style='color:#ffffff; font-weight:700;'>System Online</span>")
        ...
        self._bar_cpu = MetricBar("CPU", "#ff3333")
        self._bar_mem = MetricBar("RAM", "#ff3333")
    ...
    def _build_command_row(self) -> QHBoxLayout:
        ...
        self._input = QLineEdit()
        self._input.setPlaceholderText("Ask SATAN Echo anything...")
        self._input.setFont(QFont("Segoe UI", 10))
        ...
        mic.setStyleSheet(f"""
            ...
            QPushButton:pressed {{
                background: rgba(255,50,50, 0.40);
            }}
        """)
...
class SystemConnectivitySidebar(QFrame):
    def __init__(self, controller=None, parent=None):
        ...
        self._mk_quick_action("Γå╗ Restart SATAN Echo", QStyle.StandardPixmap.SP_BrowserReload, self._restart)
...
class SystemConnectivityPage(QWidget):
    ...
    def _provider_row(self, name: str, key: str, model: str, setting_key: str):
        ...
        if key:
            ...
            row.setStyleSheet("QFrame { background: rgba(255, 255, 255, 0.02); border: 1px solid rgba(255, 255, 255, 0.06); border-radius: 14px; } QFrame:hover { background: rgba(255,50,50, 0.04); border: 1px solid rgba(255,50,50, 0.3); }")
        r = QHBoxLayout(row)
        ...
        if key:
            ...
            icon.setStyleSheet(f"background: rgba(255,50,50, 0.12); color: {C.WHITE}; border: 1px solid rgba(255,50,50, 0.38); border-radius: 21px;")
        r.addWidget(icon)
        meta = QVBoxLayout()
        title = QLabel(name)
        title.setFont(QFont("Segoe UI", 11, QFont.Weight.Bold))
    ...
    def _build_left_column(self):
        ...
        # Mobile connect
        mobile = self._card("Mobile Connect", "Connect your phone and control SATAN Echo remotely.")
        ml = mobile.layout()
        ...
        # Startup
        startup = self._card("Startup", "Use SATAN Echo with Windows startup preferences.")
        sl = startup.layout()
        self._startup_launch_btn = self._mk_toggle("Launch SATAN Echo when Windows starts", bool(self._load_app_settings().get("show_workspace_on_startup", False)), self._toggle_startup_from_page)
        ...
        # Shortcuts & Pinning
        shortcuts = self._card("Shortcuts & Pinning", "Create shortcuts and pin SATAN Echo to your Windows system.")
        ...
        self._preview_progress.setStyleSheet("QProgressBar { background: rgba(255,255,255,0.05); border: none; border-radius: 4px; } QProgressBar::chunk { background: #ff3333; border-radius: 4px; }")
        al.addWidget(self._preview_progress)
        lay.addWidget(anim)

        # Discord bot
        discord = self._card("Discord Bot", "Mirror SATAN Echo between the app and your server.")
        dl = discord.layout()
        ...
        about = self._card("About SATAN Echo", "SATAN Echo information only.")
        ab = about.layout()
    ...
    def _build_quick_actions_box(self):
        ...
        lay = box.layout()
        actions = [
            ("Restart SATAN Echo", QStyle.StandardPixmap.SP_BrowserReload, self._restart_app),
    ...
    def _test_discord_from_page(self):
        ...
        if self._ctrl() and hasattr(self._ctrl(), "_win"):
            ...
            self._discord_msg.setText("Connected as SATAN Echo#9649" if self._discord_token.text().strip() else "Bot Offline")
    ...
    def refresh(self):
        ...
        if enabled and token:
            ...
            self._discord_msg.setText("Connected as SATAN Echo#9649")
        elif token:
    ...
    def _create_desktop_shortcut_logic(self):
        try:
            ...
            shortcut_path = desktop_dir / "SATAN Echo - Premium.lnk"
            
            # Base variables
            base_dir = Path(os.path.abspath("."))
            script_path = base_dir / "main.py"
            icon_path = base_dir / "assets" / "SATAN_Lite_Logo.ico"
            ...
            ps1_script = "\n".join([
                ...
                "$Shortcut.Description = 'Launch SATAN Echo - Premium'",
    ...
    def _pin_app_to_taskbar_logic(self):
        try:
            ...
            if res.returncode == 0:
                return True, "SATAN Echo has been pinned to your Taskbar!"
            else:
                return False, "Windows restricts programmatic taskbar pinning. Please right-click the 'SATAN Echo - Premium.lnk' shortcut on your Desktop and select 'Pin to taskbar', or drag it directly onto your taskbar."
        except Exception as e:
...
class SmartDevicesSection(QFrame):
    def __init__(self, controller=None, parent=None):
        ...
        """)
        self._open_home_btn.clicked.connect(self._open_SATAN_home)
        ...
        empty_btn = QPushButton("Open SATAN Echo Home")
        ...
        """)
        empty_btn.clicked.connect(self._open_SATAN_home)
    ...
    def _pick_color(self, device: dict[str, object]):
        color = QColorDialog.getColor(QColor("#ff3333"), self, "Pick Light Color")
        if color.isValid():
...
class SATANUI:
    def __init__(self, face_path: str, size=None, *, show_immediately: bool = True):
        ...
        self._app.setApplicationDisplayName("SATAN Echo")
        ...
        self._tray.setToolTip("SATAN Echo")
